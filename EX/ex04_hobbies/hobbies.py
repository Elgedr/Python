...
def find_most_popular_hobby(file):
    ...
    hobbies_dictionary = {}
    hobbies_list = []
    for hobbies in create_dictionary(file).values():
        hobbies_list = hobbies_list + hobbies
    for hobby in hobbies_list:
        if hobby not in hobbies_dictionary:
            hobbies_dictionary[hobby] = hobbies_list.count(hobby)

    max_value = max(hobbies_dictionary.values())
    maximum_value = [key for key, value in hobbies_dictionary.items() if value == max_value]
...
def find_least_popular_hobby(file):
    ...
    min_value = min(hobbies_dictionary.values())
    minimum_value = [key for key, value in hobbies_dictionary.items() if value == min_value]
