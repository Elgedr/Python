...
def find_how_many_pumpkins_are_needed_to_feed_animals(animal_list: list) -> int:
    ...
    """
    a = math.ceil(reduce(lambda x, y: (x + ((y.weight_range[0] + y.weight_range[1]) / 2)), list(filter(lambda x: x.diet != "carnivorous", animal_list)), 0) * 0.06 / 3 * 180)
    return a
...
def find_animals_whose_height_is_less_than(animal_list: list, height_limit: int) -> list:
    ...
    """
    a = list(filter(lambda x: x.height_range[1] < height_limit, animal_list))
    return a
...
def find_animal_with_longest_lifespan(animal_list: list) -> Animal:
    ...
    :return: Animal object
    """
    res = max(animal_list, key=lambda x: x.age_up_to)
    return res
